...
QUERY_TRACK_APPLICATION_SENT = """
    INSERT INTO analytics.tracking_application
        (id_offer, id_location, id_company, id_contact, date)
    VALUES
        ($1, $2, $3, $4, NOW())
    ON CONFLICT (id_offer, id_contact) DO NOTHING;
"""
...
def track_application_sent(
    ...
    """
    # db.execute() gère lui-même le INSERT ... ON CONFLICT DO NOTHING —
    # idempotent par construction, cohérent avec la contrainte UNIQUE de
    # analytics.tracking_application.
    db = get_db()

    db.execute(
        QUERY_TRACK_APPLICATION_SENT,
        [id_offer, id_location, id_company, id_contact],
    )
